...
Node *Scene::findNode(const int type) 
{
...
Node *Scene::findNode(const int type, const char *name) 
{
	if (name == NULL)
		return NULL;
	if (strlen(name) <= 0)
...
bool Scene::hasNode(Node *targetNode) 
{
...
Route *Scene::getRoutes() 
{
	return (Route *)mRouteList.getNodes();
}

Route *Scene::getRoute(Node *eventOutNode, Field *eventOutField, Node *eventInNode, Field *eventInField)
{
