...
Node *Node::nextTraversalByType(const int type) 
{
...
Geometry3DNode *Node::getGeometry3DNode() 
{
	for (Node *node = getChildNodes(); node != NULL; node = node->next()) {
...
bool Node::isNode(const int type) 
{
	if (getType() == type)
		return true;
	return false;
...
char *Node::getSpaceString(int nSpaces) {
...
void Node::outputHead(std::ostream& printStream, char *indentString) 
{
...
void Node::outputTail(std::ostream& printStream, char * indentString) 
{
	printStream << indentString << "}" << std::endl;
}

void Node::outputContext(std::ostream& printStream, char *indentString1, char *indentString2) 
{
...
static bool hasOutputXMLField(Field *field) 
{
...
static bool isOutputXMLFieldInSingleLine(Node *node) 
{
...
void Node::outputXMLField(std::ostream& ps, Field *field, int indentLevel, bool isSingleLine) 
{
...
void Node::outputXML(std::ostream& ps, int indentLevel) 
{
