...
void MField::outputContext(std::ostream& printStream, char *indentString1, char *indentString2) 
{
...
char *MField::getValue(String &mfBuffer)
{
	mfBuffer.clear();
	...
	for (int n=0; n<nSize; n++) {
		...
		if (0 < nString)
			mfBuffer.append(", ");
		mfBuffer.append(value.getValue());
	}
