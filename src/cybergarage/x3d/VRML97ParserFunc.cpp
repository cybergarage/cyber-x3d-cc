...
void CyberX3D::AddSFString(char *string)
{	
	switch (ParserGetCurrentNodeType()) {
...
void CyberX3D::SetDEFName(char *name)
{
	((VRML97Parser *)GetParserObject())->setDefName(name);
}

char *CyberX3D::GetDEFName(void)
{
	char *defName = ((VRML97Parser *)GetParserObject())->getDefName();
	if (defName)
